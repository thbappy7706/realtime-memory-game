<div class="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors duration-300 rounded-2xl"
     x-data="{
    init() {
    ...
    <div class="max-w-7xl mx-auto p-4 lg:p-6">
        ...
        <div class="grid lg:grid-cols-4 gap-6">
                ...
                <div class="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
                    @if ($game->status === 'new')
                        ...
                        <div class="text-center py-12">
                            <div class="w-64 h-48 mx-auto mb-6 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center">
                                <svg class="w-64 h-64 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @elseif (count($cards) > 0)
                        <!-- Game Board - Fixed 4x4 Grid -->
                        <div class="flex justify-center">
                            <div class="grid grid-cols-4 gap-6 max-w-md mx-auto">
                                @foreach ($cards as $card)
                                    <div style="width: 100px; height: 100px;"
                                        wire:click="flipCard({{$card['id']}})"
                                        class="aspect-square flex cursor-pointer items-center justify-center border-2 rounded-xl transition-all duration-300  font-bold shadow-lg hover:shadow-xl transform hover:scale-105
            @if ($card['is_matched'])
            ...
            "
                        ...
                        <!-- Game Controls -->
                        @if ($game->status === 'playing' && $game->current_player_id === auth()->id())
                            <div class="mt-6 text-center">
                                <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-4 inline-block">
                                    <p class="text-blue-800 dark:text-blue-200 font-semibold">
                                        Your turn! Click on two cards to find a match.
                                    </p>
                                    @if(count($flippedCards) > 0)
                                        <p class="text-sm text-blue-600 dark:text-blue-300 mt-1">
                                            {{ count($flippedCards) }}/2 cards selected
                                        </p>
                                    @endif
                                </div>
                            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-600">
                            <span class="text-gray-600 dark:text-gray-400">Total Pairs:</span>
                            <span class="font-semibold text-gray-900 dark:text-white">{{($game->grid_size * $game->grid_size) / 2}}</span>
                        </div>
                    ...
                    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">Cards Matched:</span>
                                <span class="font-semibold text-gray-900 dark:text-white">
                                {{ $game->cards()->where('is_matched', true)->count() }}/{{ $game->grid_size * $game->grid_size }}
                            </span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">Pairs Found:</span>
                                <span class="font-semibold text-gray-900 dark:text-white">
                                {{ $game->cards()->where('is_matched', true)->count() / 2 }}/{{ ($game->grid_size * $game->grid_size) / 2 }}
                            </span>
                            </div>
                        </div>
                    </div>
